...
function operatorPressed(op) {
    let display = document.getElementById('display-below');
    let displayText = display.textContent;

    // Update the upper display with the current number and the operator
    updateUpperDisplay(displayText + ' ' + op);

    // Existing logic for handling the operator
    ...
    if (op !== '=') {
        updateDisplay('0'); // Clear display for next number input
    } else {
        updateDisplay(result); // Show the calculated result
    }
}
...
function digitPressed(digit) {
    ...
    // Update the upper display with the ongoing expression
    let upperDisplayText = document.getElementById('display-above').textContent;
    updateUpperDisplay(upperDisplayText + digit);
}
